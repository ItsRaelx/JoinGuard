...
async def login(state_data: StateModel):
    return RedirectResponse(url=get_oauth_url(state_data.state))

@login_router.get("/callback")
async def callback(callback_data: CallbackModel):
    data = await get_code(callback_data.code)
    if data is None:
        raise HTTPException(status_code=400, detail="Failed to get access token")
    access_token = data['access_token']
    return RedirectResponse(url=f'/../main?auth={access_token}&state={callback_data.state}')
